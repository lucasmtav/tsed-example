...
let SwaggerModule = class SwaggerModule {
    ...
    $onRoutesInit() {
        this.settings.forEach((conf) => {
            ...
            if (outFile) {
                fs_1.default.writeFileSync(outFile, JSON.stringify(spec, null, 2));
            }
        });
    }
    $onReady() {
        ...
        const displayLog = (host) => {
            this.settings.forEach((conf) => {
                ...
                injector.logger.info(`[${doc || "default"}] Swagger JSON is available on ${url}${normalizePath(path_1.join(path, fileName))}`);
    ...
    getUrls() {
        return this.settings.reduce((acc, conf) => {
            ...
            if (!hidden) {
                acc.push({ url: normalizePath(path_1.join(path, fileName)), name: doc || path });
            }
            return acc;
        }, []);
    }
    ...
    createRouter(conf, urls) {
        const { disableSpec = false, fileName = "swagger.json", cssPath, jsPath, viewPath = path_1.join(__dirname, "../views/index.ejs") } = conf;
        const router = common_1.PlatformRouter.create(this.injector);
        if (!disableSpec) {
            router.get(normalizePath(path_1.join("/", fileName)), common_1.useCtxHandler(this.middlewareSwaggerJson(conf)));
        }
        if (viewPath) {
            if (cssPath) {
