...
export declare type MongooseNextCB = (err?: NativeError) => void;
export interface MongooseHookOptions {
    document?: boolean;
    query?: boolean;
    parallel?: boolean;
}
export declare type MongoosePreHookCB<T = any> = ((doc: T | MongooseDocument<T>, next: MongooseNextCB) => void) | ((doc: T | MongooseDocument<T>) => Promise<void> | void);
export declare type MongoosePostHookCB<T = any> = ((doc: T | MongooseDocument<T>, error: NativeError, next: MongooseNextCB) => void) | ((doc: T | MongooseDocument<T>, error: NativeError) => Promise<void> | void) | ((doc: T | MongooseDocument<T>, next: MongooseNextCB) => void) | ((doc: T | MongooseDocument<T>) => Promise<void> | void);
